...
# 当前脚本所在文件真实路径
cur_path = os.path.dirname(os.path.realpath(__file__))  #也就是run_main.py的真实路径
...
def get_report_file(report_path):
    '''第三步：获取最新的测试报告'''
    lists = os.listdir(report_path) ##列举report_path目录下的所有文件（名），结果以列表形式返回。
    ...
    # 找到最新生成的报告文件
    report_file = os.path.join(report_path, lists[-1]) #获取最新文件的绝对路径，列表中最后一个值,文件夹+文件名
    return report_file

def send_mail(sender, psw, receiver, smtpserver, report_file, port):
    '''第四步：发送最新的测试报告内容'''
    with open(report_file, "rb") as f:  #读文件
        mail_body = f.read()
    # 定义邮件内容
    msg = MIMEMultipart()
